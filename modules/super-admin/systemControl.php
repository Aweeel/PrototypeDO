...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ($action === 'save_settings') {
            ...
            $archivePreview = fetchAll("SELECT TOP 100 c.case_id, c.status, c.resolved_date, CONCAT(s.first_name, ' ', s.last_name) AS student_name FROM cases c LEFT JOIN students s ON s.student_id = c.student_id WHERE c.is_archived = 0 AND c.status IN ('Resolved', 'Dismissed') AND c.resolved_date IS NOT NULL AND c.resolved_date <= DATEADD(day, -?, CAST(GETDATE() AS date)) ORDER BY c.resolved_date ASC", [$days]);
            ...
            $stmt = executeQuery("UPDATE cases SET is_archived = 1, archived_at = GETDATE() WHERE is_archived = 0 AND status IN ('Resolved', 'Dismissed') AND resolved_date IS NOT NULL AND resolved_date <= DATEADD(day, -?, CAST(GETDATE() AS date))", [(int)getSystemSetting('archive_after_days', 30)]);
