...
try {
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ($action === 'toggle_maintenance' && ($_POST['ajax'] ?? '') === '1') {
            ...
            readfile($path);
            exit;
        } elseif ($action === 'restore_request') {
            $filename = basename($_POST['filename'] ?? '');
            if ($filename === '' || !is_file($backupDirectory . DIRECTORY_SEPARATOR . $filename)) throw new RuntimeException('Backup file was not found.');
            logAudit($_SESSION['user_id'], 'Backup Restore Requested', 'backup_history', null, null, ['filename' => $filename]);
            $message = 'Restore request logged. A database administrator must review and execute it.';
        }
    }
...
<section class="bg-white dark:bg-[#111827] p-6 rounded-lg shadow-sm border border-gray-200 dark:border-slate-700"><h2 class="text-xl font-bold mb-4">Backup history</h2><div class="overflow-x-auto"><table class="w-full" style="table-layout: fixed"><thead class="bg-gray-100 dark:bg-slate-800 border-b border-gray-200 dark:border-slate-700"><tr><th class="px-6 py-3 text-left text-xs font-medium text-gray-600 dark:text-gray-400 uppercase">File</th><th class="px-6 py-3 text-left text-xs font-medium text-gray-600 dark:text-gray-400 uppercase">Created</th><th class="px-6 py-3 text-left text-xs font-medium text-gray-600 dark:text-gray-400 uppercase">Size</th><th class="px-6 py-3 text-left text-xs font-medium text-gray-600 dark:text-gray-400 uppercase">Actions</th></tr></thead><tbody class="divide-y divide-gray-200 dark:divide-slate-700"><?php foreach ($historyEntries as $backup): ?><tr class="hover:bg-gray-50 dark:hover:bg-slate-800/50"><td class="px-6 py-4 text-sm"><?= htmlspecialchars($backup['filename'] ?? '') ?></td><td class="px-6 py-4 text-sm"><?= htmlspecialchars($backup['created_at'] ?? '') ?></td><td class="px-6 py-4 text-sm"><?= number_format(((int)($backup['size'] ?? 0)) / 1024, 1) ?> KB</td><td class="px-6 py-4"><div class="flex gap-2"><a class="px-3 py-2 border border-gray-300 dark:border-slate-600 rounded-lg hover:bg-gray-100 dark:hover:bg-slate-700 text-sm" href="<?= BASE_URL ?>/assets/backups/<?= rawurlencode($backup['filename'] ?? '') ?>" download>Download</a><form method="post" onsubmit="return confirm('Log a restore request for this backup?')"><input type="hidden" name="action" value="restore_request"><input type="hidden" name="filename" value="<?= htmlspecialchars($backup['filename'] ?? '') ?>"><button class="px-3 py-2 bg-amber-600 text-white rounded-lg text-sm hover:bg-amber-700">Restore request</button></form></div></td></tr><?php endforeach; ?><?php if (!$history): ?><tr><td colspan="4" class="px-6 py-8 text-center text-gray-500 dark:text-gray-400">No backups generated yet.</td></tr><?php endif; ?></tbody></table></div></section>
